...
int main() {
	...
	const int nTloops = 81;							// number of training loops -- stack overflow error when set to 100
	const int nPairs = 36;							// number of training pairs

	const int nNeurIn = 35;							// no. of input neurons
	const int nNeurMid = 20;						// no. of hidden layer neurons
	const int nNeurOut = nPairs;					// no. of output neurons

	double ErrLog[nNeurOut + 1][nTloops * nPairs] = { 0 }; // error log

	ofstream myfile;								// ouptut file
	myfile.open("C:\\Users\\dougl\\source\\repos\\7x5_AlphaNum_ANN\\Output_Folder\\errorlog.txt", ios::trunc);
	...
	*/
	...
	for (int Tloop = 0; Tloop < nTloops; Tloop++) {							// iterates through training loops
		...
		// Iterating Through all All Training Pairs
		for (int Curr_Pair = 0; Curr_Pair < nPairs; Curr_Pair++) {
			for (int c = 0; c < nNeurIn; c++) {								// input pair definition
				NeurIn[c][1] = input_read_array[(nPairs * c) + Curr_Pair];
			}
			for (int d = 0; d < nNeurOut; d++) {							// target definition
				if (d == Curr_Pair) {
					Target[d] = 1;
				}
				else {
			...
			ErrLog[0][((Tloop * nPairs) + Curr_Pair)] = ((Tloop * nPairs) + Curr_Pair);
			for (int o = 0; o < nNeurOut; o++) {
				double squish = ((1 - NeurOut[o][1]) * NeurOut[o][1]);
				NeurOut[o][2] = (Target[o] - NeurOut[o][1]) * squish;
				ErrLog[o + 1][((Tloop * nPairs) + Curr_Pair)] = NeurOut[o][2];

				//Store metric of initial error
				if (Tloop == 0) {
					init_total_error_sq += pow(NeurOut[o][2], 2);
				}

				//Store metric of final error
				if (Tloop == nTloops - 1) {
	...
	cout << "\nThe initial magnitude of all errors is: " << init_total_error_sq << endl;
	cout << "The final magnitude of all errors is: " << fin_total_error_sq << endl;
...
void set_initial_weights(double layer_in_mid[], double layer_mid_out[], int input_neurons, int midlayer_neurons, int output_neurons) {
	...
	initial_weights.open("C:\\Users\\dougl\\source\\repos\\7x5_AlphaNum_ANN\\Output_Folder\\initial_weights.txt", ios::trunc);
...
void outro(int training_loops, int input_neurons, int midlayer_neurons, int output_neurons, double layer_in_mid[], double layer_mid_out[]) {
	...
	final_weights.open("C:\\Users\\dougl\\source\\repos\\7x5_AlphaNum_ANN\\Output_Folder\\final_weights.txt", ios::trunc);
	...
	cout << "...\\7x5_AlphaNum_ANN\\Output_Folder\\..." << endl;

}
void to_matlab(int training_loops, int input_neurons, int midlayer_neurons, int output_neurons, double layer_in_mid[], double layer_mid_out[]) {
	// outputs sent to txt file
	ofstream matlab_outfile;								// ouptut file
	matlab_outfile.open("C:\\Users\\dougl\\source\\repos\\7x5_AlphaNum_ANN\\Output_Folder\\to_matlab.txt", ios::trunc);
